...
parameters = {'layer_sizes':([10],[50],[50,20],[30,20,10]), 'activation_function':['sigmoid', 'tanh', 'relu'], 'alpha':[0, 0.01, 0.03, 0.1], 'epochs':[10,40]}
nnTensor = classes_jolynde.Neural_TensorFlow(batch_size = 32)
clf = GridSearchCV(nnTensor, parameters, scoring = 'accuracy', cv=5, verbose = 8)
...
area_ratio_nn_Tensor = area_ratio(ytest, nnBestTensor.predict(Xtest)[:,1], plot = True)
print('Area ratio: %g' % area_ratio_nn_Tensor)
print('Confusion matrix for TensorFlow neural network:')
conf = confusion_matrix(ytest, nnBestTensor.predict_classes(Xtest)[:,1])
print(conf)
